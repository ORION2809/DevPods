...
|---|---------------|------------|----------------|-------|
| 5 | Ingestion orchestrator | `src/core/ingestion/pipeline.ts`, `pipeline-phases/runner.ts` | `src/intelligence/gitnexus/ingest-workspace.ts` | ✅ Walker → parser → graph writer wired into single pipeline |
...
### Phase 3a — Ingestion Orchestrator ✅ (2026-06-08)

- `graph-writer.ts` — batch Cypher INSERT for nodes (multi-node CREATE) and relationships (UNWIND + MATCH)
- `ingest-workspace.ts` — full pipeline: walk → hash-diff → read → parse → write → save-meta → manifest
- `GitNexusIntelligenceLayer.indexWorkspace()` — public API to trigger indexing
- Rebuild strategy: full rebuild on any change (MVP), with hash-diff skip when unchanged
- Clears graph via `DELETE` relationships then `DELETE` nodes (Kuzu-compatible, no `DETACH DELETE`)
- Structure phase: File + Folder nodes with CONTAINS edges
- Symbol phase: Function/Class/Interface/Method/Property/CodeElement nodes with DEFINES edges
